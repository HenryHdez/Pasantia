...
vel_rob1_mov = [0.1, 0.0, 0.05]  # Velocidades ajustadas: Vx, Vy, Om (ajustables)
...
def mover_robot_repetidamente(ip, interval, vel_rob1):
    while True:
        ...
        actualizar_posicion(0.5)  # Actualiza la posición del robot
...
# Función para actualizar el gráfico de la trayectoria
def actualizar_grafico_trayectoria(frame, posiciones_x, posiciones_y, trayectoria, robot_dot):
    # Agregar la nueva posición a las listas
    posiciones_x.append(robot_position[0])
    posiciones_y.append(robot_position[1])

    # Actualizar los datos de la línea del recorrido y la posición actual
    trayectoria.set_data(posiciones_x, posiciones_y)
    robot_dot.set_data(robot_position[0], robot_position[1])

    return trayectoria, robot_dot

# Función para actualizar el gráfico en cada frame (sensores)
def actualizar_grafico_sensores(frame, sensor_data_container, sensor_lines, sensor_positions):
...
def configurar_figura(sensor_data_container):
    ...
    # Función de animación que actualiza ambos gráficos
    def actualizar_graficos(frame):
        actualizar_grafico_trayectoria(frame, posiciones_x, posiciones_y, trayectoria, robot_dot)
        actualizar_grafico_sensores(frame, sensor_data_container, sensor_lines, sensor_positions)

    # Iniciar la animación para ambos gráficos en intervalos regulares
    anim = FuncAnimation(fig, actualizar_graficos, interval=500)
